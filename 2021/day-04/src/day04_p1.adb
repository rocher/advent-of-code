...
begin
   ...
   loop
      ...
      loop
         ...
         Has_Bingo := Board.Bingo;

         Board_Elt := Boards_Lists.Next (Board_Elt);
         exit Process_Boards when Has_Bingo or
           not Boards_Lists.Has_Element (Board_Elt);
      end loop Process_Boards;

      Number_Elt := Numbers_Lists.Next (Number_Elt);
      exit Process_Numbers when Has_Bingo or
        not Numbers_Lists.Has_Element (Number_Elt);
   end loop Process_Numbers;

   Put_Line ("Answer:" & Natural'Image (Number * Board.Sum_Unmarked));
end Day04_P1;
