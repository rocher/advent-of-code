...
procedure Day12_P2 is
   ...
   function Repeated_Small_Cave (List : Cave_List_Type) return Boolean is
      package Cave_Exists_Package is new Hashed_Maps
        (Cave_Type, Boolean, Cave_Hash, "=", "=");
      use Cave_Exists_Package;
      Cave_Exists : Cave_Exists_Package.Map;
   begin
      for Cave of List loop
         if Is_Small (Cave) then
            if Cave_Exists.Find (Cave) = Cave_Exists_Package.No_Element then
               Cave_Exists.Insert (Cave, True);
            else
               return True;
            end if;
         end if;
      ...
      return False;
   end Repeated_Small_Cave;

   function Visitable
     (Visited_Caves : Cave_List_Type; Cave : Cave_Type) return Boolean
   is
   begin
      if Is_Big (Cave) then -- or Cave = End_Cave then
         --  big caves can be visited any number of times
         return True;
      else
         --  small caves can be visited only once if another small cave is
         --  already visited twice, otherwise can be visited two times
         if Repeated_Small_Cave (Visited_Caves) then
   ...
   Cave_Path_Ref : Cave_List_Package.Cursor;
   Src, Dst      : Cave_Type;
